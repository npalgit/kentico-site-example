...
public partial class CMSWebParts_OnlineMarketing_MyContacts : CMSAbstractWebPart
{
    ...
    {
        // Check permissions
        bool siteContactsAllowed = CMSContext.CurrentUser.IsAuthorizedPerResource("CMS.ContactManagement", "ReadContacts");
        bool globalContactsAllowed = CMSContext.CurrentUser.IsAuthorizedPerResource("CMS.ContactManagement", "ReadGlobalContacts") && SettingsKeyProvider.GetBoolValue(CMSContext.CurrentSiteName + ".CMSCMGlobalContacts");
        ...
        if (siteContactsAllowed && !globalContactsAllowed)
        {
            where = "ContactSiteID>0";
        } else if (globalContactsAllowed && !siteContactsAllowed)
        {
            where = "(ContactSiteID=0 OR ContactSiteID IS NULL)";
        }

        int currentUserID = CMSContext.CurrentUser.UserID;
        gridElem.Visible = true;
        gridElem.WhereCondition = SqlHelperClass.AddWhereCondition("ContactOwnerUserID=" + currentUserID + " AND ContactMergedWithContactID IS NULL", where);
