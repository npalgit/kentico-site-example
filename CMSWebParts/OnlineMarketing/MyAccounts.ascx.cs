...
public partial class CMSWebParts_OnlineMarketing_MyAccounts : CMSAbstractWebPart
{
    ...
    protected void SetupControl()
    {
        ...
        bool globalAccountsAllowed = currentUser.IsAuthorizedPerResource("CMS.ContactManagement", "ReadGlobalAccounts") && SettingsKeyProvider.GetBoolValue(CMSContext.CurrentSiteName + ".CMSCMGlobalContacts");
        ...
        if (siteAccountsAllowed && !globalAccountsAllowed)
        {
            where = "AccountSiteID>0";
        }
        else if (globalAccountsAllowed && !siteAccountsAllowed)
        {
            where = "(AccountSiteID=0 OR AccountSiteID IS NULL)";
        }

        int currentUserID = currentUser.UserID;
        gridElem.Visible = true;
        gridElem.WhereCondition = SqlHelperClass.AddWhereCondition("AccountOwnerUserID=" + currentUserID + " AND AccountMergedWithAccountID IS NULL", where);
