...
public partial class CMSModules_Content_CMSDesk_Edit_ImageEditor : CMSModalPage
{
    ...
    private static string GetRefresh()
    {
        if (QueryHelper.GetBoolean("refresh", false))
        {
            ...
            {
                return String.Format("if (wopener.imageEdit_AttachmentRefresh) {{ wopener.imageEdit_AttachmentRefresh('attachmentguid|{0}'); }} else {{ InitRefresh({1}, false, false, '{0}', 'refresh'); }}", ScriptHelper.GetString(attachmentGuid, false), ScriptHelper.GetString(clientid));
            }
        }
