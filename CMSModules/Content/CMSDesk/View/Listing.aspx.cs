...
function RefreshGrid()
{
    ...
    protected override void OnPreRender(EventArgs e)
    {
        ...
        else
        {
            if (FlagsControls.Count != 0)
            {
                ...
                bool checkPermissions = tree.CheckDocumentUIPermissions(currentSiteName);
                if (checkPermissions)
                {
    ...
    protected void btnShow_OnClick(object sender, EventArgs e)
    {
        ...
        CurrentWhereCondition = gridDocuments.WhereCondition;
        pnlGrid.Update();
    }
    ...
    protected void gridDocuments_OnBeforeDataReload()
    {
        ...
        if (node != null)
        {
            ...
            if (si != null)
            {
                ...
                string columns = SqlHelperClass.MergeColumns(TreeProvider.SELECTNODES_REQUIRED_COLUMNS, @"DocumentLastVersionName, DocumentName, NodeParentID,
                ...
                {
                    // Get documents
                    ds = DocumentHelper.GetDocuments(currentSiteName, TreeProvider.ALL_DOCUMENTS, TreeProvider.ALL_CULTURES, true, null, whereCondition, "NodeOrder ASC, NodeName ASC, NodeAlias ASC", -1, false, gridDocuments.TopN, columns, tree);
                }
                else
                {
