...
public partial class CMSModules_Content_CMSDesk_Delete : CMSContentPage
{
            ...
            if (!RequestHelper.IsCallback())
            {
                ...
                if (nodeIds.Count > 0)
                {
                    if (nodeIds.Count == 1)
                    {
                        ...
                        if (node != null)
                        {
                            if (!IsUserAuthorizedToDeleteDocument(node, currentSite.SiteName))
                            {
                            ...
                            chkDestroy.Visible = CanDestroy(node, currentSite.SiteName);
                        ...
                        if (!DataHelper.DataSourceIsEmpty(ds))
                        {
                            ...
                            foreach (DataTable table in ds.Tables)
                            {
                                foreach (DataRow dr in table.Rows)
                                {
                                    ...
                                    canDestroy = CanDestroy(node, currentSite.SiteName) && canDestroy;
        ...
        try
        {
            ...
            if (!DataHelper.DataSourceIsEmpty(ds))
            {
                ...
                foreach (DataRow nodeRow in ds.Tables[0].Rows)
                {
                    ...
                    if (IsUserAuthorizedToDeleteDocument(node, currentSite.SiteName) && (CanDestroy(node, siteName) || !chkDestroy.Checked))
                    {
                        // Delete the document
                        if (parentId <= 0)
                        {
    ...
    private void LogExceptionToEventLog(Exception ex)
    {
        ...
        log.LogEvent(EventLogProvider.EVENT_TYPE_ERROR, DateTime.Now, "Content", "DELETEDOC", currentUser.UserID, currentUser.UserName, 0, null, HTTPHelper.GetUserHostAddress(), EventLogProvider.GetExceptionLogMessage(ex), currentSite.SiteID, HTTPHelper.GetAbsoluteUri());
    ...
    private bool CanDestroy(TreeNode node, string siteName)
    {
        return (currentUser.IsAuthorizedPerDocument(node, NodePermissionsEnum.Destroy, siteName) == AuthorizationResultEnum.Allowed);
    }
