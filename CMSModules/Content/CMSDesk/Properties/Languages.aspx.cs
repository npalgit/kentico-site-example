        ...
        if (nodeId != 0)
        {
            if (!dataReloaded)
            {
                ...
                if (node != null)
                {
                    ...
                    DataSet documentsDS = DocumentHelper.GetDocuments(currentSiteName, node.NodeAliasPath, TreeProvider.ALL_CULTURES, false, null, null, null, -1, false, topN, TreeProvider.SELECTNODES_REQUIRED_COLUMNS + ", DocumentModifiedWhen, DocumentLastPublished, DocumentName, VersionNumber, Published", tree);
