...
public partial class CMSModules_Content_CMSDesk_Properties_LinkedDocs : CMSPropertiesPage
{
    ...
    protected void gridDocs_OnBeforeDataReload()
    {
        if (nodeId > 0)
        {
            if (node != null)
            {
                ...
                string columns = SqlHelperClass.MergeColumns(TreeProvider.SELECTNODES_REQUIRED_COLUMNS, "NodeAliasPath, SiteName, NodeParentID, DocumentName, DocumentNamePath, ClassDisplayName");
