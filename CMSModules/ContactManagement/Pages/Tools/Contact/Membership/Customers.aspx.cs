        ...
        if (ContactHelper.AuthorizedModifyContact(ci.ContactSiteID, true))
        {
            ...
            if (!String.IsNullOrEmpty(values))
            {
                ...
                foreach (string customerId in customerIds)
                {
                    ...
                    customerIdInt = ValidationHelper.GetInteger(customerId, 0);
                    if (customerIdInt <= 0)
                    {
                        continue;
                    }
                    // Add new relation
                    MembershipInfoProvider.SetRelationship(customerIdInt, MemberTypeEnum.EcommerceCustomer, contactId, contactId, true);
                }
