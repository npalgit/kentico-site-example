...
public partial class CMSModules_ContactManagement_Pages_Tools_Contact_Membership_Subscribers : CMSContactManagementContactsPage
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        modifyAllowed = ContactHelper.AuthorizedModifyContact(ci.ContactSiteID, true);
        ...
        this.CurrentMaster.HeaderActionsPlaceHolder.Visible = !(globalContact || mergedContact);
    ...
    protected void Page_PreRender(object sender, EventArgs e)
    {
        ...
        if (!showFullName)
        {
            DataView dv = ((DataView)gridElem.GridView.DataSource);
            if (!DataHelper.DataSourceIsEmpty(dv))
            {
                DataRow[] dr = dv.Table.Select("ContactMergedWithContactID > 0");
                showFullName = (dr.Length > 0);
            }
        }
        ...
        if (ContactHelper.AuthorizedModifyContact(ci.ContactSiteID, true))
        {
            ...
            if (!String.IsNullOrEmpty(values))
            {
                ...
                foreach (string subscriberId in subscriberIds)
                {
                    ...
                    subscriberIdInt = ValidationHelper.GetInteger(subscriberId, 0);
                    if (subscriberIdInt <= 0)
                    {
                        continue;
                    }
                    // Add new relation
                    MembershipInfoProvider.SetRelationship(subscriberIdInt, MemberTypeEnum.NewsletterSubscriber, contactId, contactId, true);
                }
