...
public partial class CMSModules_ContactManagement_Pages_Tools_Contact_Add_Account_Dialog : CMSModalPage
{
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if ((QueryHelper.ValidateHash("hash", "selectedvalue")) && (parameters != null))
        {
            ...
            if (siteid != -1)
            {
                AccountHelper.AuthorizedReadAccount(siteid, true, isSitemanager);
                if (AccountHelper.AuthorizedModifyAccount(siteid, false, isSitemanager) || ContactHelper.AuthorizedModifyContact(siteid, false, isSitemanager))
                {
                    ...
                    contactRoleSelector.IsSiteManager = isSitemanager;
