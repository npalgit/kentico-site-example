...
public partial class CMSModules_ContactManagement_Pages_Tools_Contact_Subscriber_Contacts : CMSNewsletterSubscribersPage
{
    ...
    {        
        ...
        UniGrid.WhereCondition = "ContactID IN (SELECT ContactGroupMemberRelatedID FROM OM_ContactGroupMember WHERE ContactGroupMemberContactGroupID = " + QueryHelper.GetInteger("groupid", 0) + " AND ContactGroupMemberType = 0) AND ContactSiteID = " + CMSContext.CurrentSiteID;
    ...
    private static void CheckAuthorization()
    {
        ...
        if (!CMSContext.CurrentUser.IsAuthorizedPerResource("om.contactmanagement", "managecontacts"))
        {
            RedirectToCMSDeskAccessDenied("om.contactmanagement", "managecontacts");
        }
    }
