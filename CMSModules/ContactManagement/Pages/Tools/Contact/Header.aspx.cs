...
public partial class CMSModules_ContactManagement_Pages_Tools_Contact_Header : CMSContactManagementContactsPage
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        // Show contact groups
        bool readSiteContactGroups = ((ci.ContactSiteID > 0) && ContactGroupHelper.AuthorizedReadContactGroup(ci.ContactSiteID, false));
        bool readGlobalContactGroups = ContactGroupHelper.AuthorizedReadContactGroup(UniSelector.US_GLOBAL_RECORD, false);
        if (readSiteContactGroups || readGlobalContactGroups)
        {
        ...
        // Show scoring tab only for users that can actually read scores
        if (ci.ContactSiteID > 0 && user.IsAuthorizedPerResource("CMS.Scoring", "Read"))
        {
