...
<cms:UniGrid runat="server" ID="gridElem" ObjectType="om.activitylist"
    Columns="ActivityID,ContactFullNameJoined,ActivityTitle,ActivityType,ActivityCreated,ActivityIPAddress,ActivitySiteID"
    IsLiveSite="false">
    <GridActions Parameters="ActivityID">
        <ug:Action Name="view" ExternalSourceName="view" Caption="$General.View$" Icon="View.png" />
        <ug:Action Name="#delete" ExternalSourceName="delete" CommandArgument="ActivityID" Caption="$General.Delete$" Icon="Delete.png" Confirmation="$General.ConfirmDelete$" ModuleName="CMS.ContactManagement" Permissions="ManageActivities" />
    </GridActions>
    <GridColumns>
        <ug:Column Source="ActivityTitle" Caption="$om.activity.gridtitle$" Wrap="false" />
        <ug:Column Source="ActivityType" ExternalSourceName="acttype" Caption="$general.type$" Wrap="false">
            <Tooltip Source="ActivityType" ExternalSourceName="acttypedesc" />
        </ug:Column>
        <ug:Column Source="ContactFullNameJoined" Caption="$om.activity.contactname$" Wrap="false" Name="contactname" />
        <ug:Column Source="ActivityIPAddress" Caption="$om.activity.ipaddress$" Wrap="false" Name="ipaddress" />
        <ug:Column Source="ActivityCreated" Caption="$om.activity.activitytime$" Wrap="false" />
        <ug:Column Source="ActivitySiteID" ExternalSourceName="#sitenameorglobal" Caption="$general.sitename$" Wrap="false" Name="sitename" />
        <ug:Column Width="100%" />
    </GridColumns>
