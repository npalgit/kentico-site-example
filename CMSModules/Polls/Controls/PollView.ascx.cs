...
public partial class CMSModules_Polls_Controls_PollView : CMSUserControl
{
    ...
    protected bool HasPermission()
    {
        ...
        if (pi != null)
        {
            ...
            {
                // Get user roles of the current site
                DataTable roles = UserInfoProvider.GetUserRoles(CMSContext.CurrentUser, " (SiteID IS NULL OR SiteID=" + CMSContext.CurrentSiteID + ")", null, 0, "RoleName");

                string role = null;
                foreach (DataRow row in roles.Rows)
                {
                    role = ValidationHelper.GetString(row["RoleName"], "").ToLower();
                    if (pi.AllowedRoles.Contains(role) && RoleInfoProvider.RoleExists(role, CMSContext.CurrentSiteName))
                    {
                        toReturn = true;
                    }
                }
