...
public partial class CMSModules_Newsletters_Tools_Newsletters_Newsletter_Issue_List : CMSNewsletterNewslettersPage
{
    ...
    private static string GetOpenedEmails(DataRowView rowView)
    {
        int openedEmails = ValidationHelper.GetInteger(DataHelper.GetDataRowViewValue(rowView, "IssueOpenedEmails"), 0);

        if (openedEmails > 0)
        {
            return string.Format(@"<a href=""#"" onclick=""ShowOpenedBy({0})"">{1}</a>", ValidationHelper.GetString(DataHelper.GetDataRowViewValue(rowView, "IssueID"), "0"), openedEmails);
        }
        else
        {
            return "0";
