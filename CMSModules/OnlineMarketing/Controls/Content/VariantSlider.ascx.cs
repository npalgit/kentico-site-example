...
public partial class CMSModules_OnlineMarketing_Controls_Content_VariantSlider : CMSAbstractPortalUserControl, IPostBackEventHandler
{
    #region "Variables"
...
function OpenMenuAddWidgetVariant(menuPositionEl, targetId) {
            ...
            string removeVariantScript = @"
                function RemoveVariantPostBack_" + uniqueCode + @"() {
                    if (confirm('" + ResHelper.GetString("variantslider.removeconfirm") + @"')) {
                        document.getElementById('" + hdnRemoveVariantId.ClientID + @"').value = GetCurrentVariantId('" + uniqueCode + @"');
                        SetVariant('" + uniqueCode + "', 0);"
                        + ControlsHelper.GetPostBackEventReference(this, "remove") + @";
                    }
                }";
    ...
    public void RaisePostBackEvent(string eventArgument)
    {
        ...
        if (arg == "remove")
        {
            ...
            int variantId = ValidationHelper.GetInteger(hdnRemoveVariantId.Value, 0);
            string uniqueCode = string.Empty;
            int documentId = 0;
