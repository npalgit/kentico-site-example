...
public partial class CMSModules_RecycleBin_Controls_RecycleBin : CMSUserControl
{
    ...
    private void PrepareGrid()
    {
        ...
        int userId = GetUserId(currentUser.UserID);
        ...
        try
        {
            ...
            switch (settings.CurrentWhat)
            {
                case What.AllDocuments:
                    ...
                    DateTime modifiedTo = DateTimeHelper.ZERO_TIME;
                    SetDocumentAge(ref modifiedFrom, ref modifiedTo);
                    recycleBin = VersionHistoryInfoProvider.GetRecycleBin((mSelectedSite != null) ? mSelectedSite.SiteID : 0, GetUserId(settings.User.UserID), GetWhereCondition(string.Empty), "CMS_VersionHistory.DocumentNamePath ASC", -1, "VersionHistoryID, CMS_VersionHistory.DocumentNamePath, CMS_VersionHistory.VersionDocumentName", modifiedFrom, modifiedTo);
                    break;
    ...
    private void EmptyBin(object parameter)
    {
        ...
        recycleBin = VersionHistoryInfoProvider.GetRecycleBin((mSelectedSite != null) ? mSelectedSite.SiteID : 0, GetUserId(currentUserInfo.UserID), where, "DocumentNamePath ASC", -1, null, modifiedFrom, modifiedTo);
        ...
        try
        {
            if (!DataHelper.DataSourceIsEmpty(recycleBin))
            {
                ...
                foreach (DataRow dr in recycleBin.Tables[0].Rows)
                {
                    ...
                    // Check permissions
                    TreeNode tn = null;
                    if (!IsAuthorizedPerDocument(versionHistoryId, "Destory", currentUser, out tn, verMan))
                    {
        ...
        switch (sourceName)
        {
            ...
            case "nodesiteid":
                {
                    ...
                    return (si != null) ? si.DisplayName : string.Empty;
                }
            ...
            case "versionclassid":
                ...
                if (classId > 0)
                {
                    ...
                    return (dci != null) ? dci.ClassDisplayName : GetString("general.na");
                }
    ...
    /// <summary>
    /// Gets user identifier for obtaining correct version history.
    /// </summary>
    /// <param name="defaultUserId">User identifier to use in case of CMS desk and single site mode</param>
    /// <returns></returns>
    private int GetUserId(int defaultUserId)
    {
        return (IsSingleSite && IsCMSDesk) ? defaultUserId : 0;
    }
