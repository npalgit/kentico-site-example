...
public partial class CMSModules_Widgets_InlineControl_InlineWidget : InlineUserControl
{
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        {
            decodedProperties[param.Key] = HttpUtility.UrlDecode(param.Value.ToString());
        }
        ...
        try
        {
            ...
            foreach (DataColumn column in dr.Table.Columns)
            {
                ...
                if (value is string)
                {
                    ...
                    // Check whether string value is defined
                    if (!String.IsNullOrEmpty(strValue))
                    {
                        ...
                        if (publicFields.IndexOf(";" + columnName + ";") < 0)
                        {
                            ...
                            // Resolve macros
                            value = control.ContextResolver.ResolveMacros(strValue, avoidInjection);
                        }
                    }
                }
