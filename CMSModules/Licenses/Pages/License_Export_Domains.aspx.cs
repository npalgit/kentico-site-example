        ...
        else
        {
            try
            {
                ...
                {                   
                    ...
                    {
                        string url = AzureHelper.GetDownloadPath(downloadPath + "Export/" + fileName);
                        string hash = ValidationHelper.GetHashString(URLHelper.GetQuery(url));
                        href = URLHelper.ResolveUrl(URLHelper.AppendQuery(url, "hash=" + hash));
                    }

                    link = "<a href=\"" + href + "\" target=\"_blank\">" + GetString("license.export.download") + "</a>";
                }
                else
                {
                    location = path;
