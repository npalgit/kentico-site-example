            ...
            if (!DataHelper.DataSourceIsEmpty(recycleBin))
            {
                foreach (DataRow dr in recycleBin.Tables[0].Rows)
                {
                    ...
                    else
                    {
                        ...
                        LogContext.LogEvent(EventLogProvider.EVENT_TYPE_INFORMATION, DateTime.Now, "Objects", "DESTROYOBJECT", currentUserInfo.UserID, currentUserInfo.UserName, 0, null, HTTPHelper.GetUserHostAddress(), ResHelper.GetString("objectversioning.Recyclebin.objectdestroyed"), (currentSite != null) ? currentSite.SiteID : 0, HTTPHelper.GetAbsoluteUri(), HTTPHelper.MachineName, HTTPHelper.GetUrlReferrer(), HTTPHelper.GetUserAgent());
                    }
                }
    ...
    protected object ugRecycleBin_OnExternalDataBound(object sender, string sourceName, object parameter)
    {
        ...
        switch (sourceName)
        {
            ...
            case "versionobjecttype":
                ...
                return GetString("ObjectType." + objType.Replace(".", "_"));
        }
