...
public partial class CMSModules_MediaLibrary_Controls_MediaLibrary_MediaFileMultipleImport : CMSAdminControl
{
    ...
    private void HandleSingleMediaFile()
    {
        ...
        try
        {
            ...
            int indexOfDel = this.ImportFilePaths.IndexOf('|');
            string currFilePath = this.ImportFilePaths.Substring(0, indexOfDel);
            if (currFilePath.Trim() != "")
            {
                string filePath = (string.IsNullOrEmpty(this.FolderPath)) ? currFilePath : DirectoryHelper.CombinePath(this.FolderPath, currFilePath);
            ...
            string filePaths = this.ImportFilePaths.Remove(0, indexOfDel + 1);
            this.ImportFilePaths = filePaths;

            wasLast = (this.ImportFilePaths == "");
            if (wasLast)
            {
                FinishImport();
                return;
    ...
    private void UpdateImportForm()
    {
        ...
        if (indexOfDel >= 0)
        {
            string nextFileName = this.ImportFilePaths.Substring(0, indexOfDel);
