...
public partial class CMSModules_MediaLibrary_Controls_MediaLibrary_MediaLibrary : CMSAdminItemsControl
{
    ...
    /// </summary>
    private void HandleAzurePrepare()
    {
    ...
    /// <summary>
    /// Handles actions coming from mass action control.
    /// </summary>
    /// <param name="argument">Type of an action occurring and files context</param>
    private void HandleMassAction(string argument)
    {
        ...
        if (argArr.Length == 2)
        {
            ...
            if (LibraryInfo != null)
            {
                ...
                string fileDir = DirectoryHelper.CombinePath(libPath, LastFolderPath);
                ...
                switch (actionType.ToLower())
                {
                    case "copy":
                        ...
                        if (!allFiles)
                        {
                            if (mediaView.SelectedItems.Count > 0)
                            {
                                ...
                                {
                                    string filePath = DirectoryHelper.EnsurePathBackSlash(fileDir);
                                ...
                                copyFiles = CMSDialogHelper.EscapeUrlParameter(builder.ToString());

                                // Check if some files selected in full listing mode
                                if (IsFullListingMode && String.IsNullOrEmpty(copyFiles))
                                {
                        ...
                        copyFolder = CMSDialogHelper.EscapeUrlParameter(MediaLibraryHelper.EnsurePath(LastFolderPath));
                        ...
                        // Add security hash
                        url = ResolveUrl(URLHelper.AddParameterToUrl(url, "hash", QueryHelper.GetHash(url, false)));
                        ScriptHelper.RegisterStartupScript(Page, typeof(Page), "modalCopy", ScriptHelper.GetScript("modalDialog(" + ScriptHelper.GetString(url) + ", 'CopyFiles', '90%', '70%');"));
                        break;
                    ...
                    case "move":
                        ...
                        if (!allFiles)
                        {
                            if (mediaView.SelectedItems.Count > 0)
                            {
                                ...
                                moveFiles = CMSDialogHelper.EscapeUrlParameter(builder.ToString());

                                // Check if some files selected in full listing mode
                                if (IsFullListingMode && String.IsNullOrEmpty(moveFiles))
                                {
                                    DisplayError(GetString("general.noitems"));
                                    break;
                                }
                        ...
                        moveFolder = CMSDialogHelper.EscapeUrlParameter(MediaLibraryHelper.EnsurePath(LastFolderPath));

                        url = GetCopyMoveDialogUrl();
                        url += "?action=move&folderpath=" + moveFolder + "&libraryid=" + LibraryID + "&files=" + moveFiles + (allFiles ? "&allFiles=1" : "");
                        // Add security hash
                        url = ResolveUrl(URLHelper.AddParameterToUrl(url, "hash", QueryHelper.GetHash(url, false)));
                        ScriptHelper.RegisterStartupScript(Page, typeof(Page), "modalMove", ScriptHelper.GetScript("modalDialog(" + ScriptHelper.GetString(url) + ", 'MoveFiles', '90%', '70%');"));
                        break;
                    ...
                    case "delete":
                        ...
                        else
                        {
                            ...
                            {
                                DisplayError(String.Format("{0}: {1}", GetString("media.delete.failed"), ex.Message));
                            }
                            finally
                            {
                                mediaView.ResetListSelection();
                        ...
                        if (!allFiles)
                        {
                            if (mediaView.SelectedItems.Count > 0)
                            {
                                ...
                                foreach (string fileName in selectedItems)
                                {
                                    ...
                                    if (File.Exists(filePath + fileName) && (FileIsNotInDatabase(fileName) == null))
                                    {
                                        numberOfFiles++;
                                        builder.Append(fileName);
                                        builder.Append("|");
                                ...
                                // Check if some files selected in full listing mode
                                if (IsFullListingMode && (numberOfFiles == 0))
                                {
                                    DisplayError(GetString("general.noitems"));
                                    break;
                                }
    ...
    protected string multipleImport_GetItemUrl(string fileName)
    {
        ...
        {
            DataSet fileData = GetFileSystemDataSource("FileName LIKE '" + DataHelper.EscapeLikeQueryPatterns(fileName, true, true, true).Replace("'", "''") + "'");
