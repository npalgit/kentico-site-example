...
public partial class CMSModules_MediaLibrary_Controls_Dialogs_MediaView : MediaView
{
    ...
    /// <summary>
    /// Gets or sets the value which determineds whtether to show the Azure button or not.
    /// </summary>
    public bool ShowAzureButton { get; set; }
    ...
    protected override void OnPreRender(EventArgs e)
    {
        ...
        if (ShowAzureButton)
        {
            plcAzure.Visible = true;
            imgAzurePrepare.ImageUrl = ResolveUrl(GetImageUrl("CMSModules/CMS_Content/Dialogs/importprepare.png", IsLiveSite));
            mSaveText = GetString("dialogs.mediaview.azureprepare");
            lnkAzurePrepare.Attributes["onclick"] = "ShowIndicator(true, true); SetAction('azureprepare', ''); RaiseHiddenPostBack(); return false;";
    ...
    private void SetupControls()
    {
        ...
        //Ensure azure button
        ShowAzureButton = AzureHelper.IsRunningOnAzure;
...
function SetSelectAction(argument) {
    ...
    public string GetItemUrl(MediaFileInfo fileInfo, SiteInfo site, Guid fileGuid, string fileName, string fileExtension, string filePath, bool isPreview, int height, int width, int maxSideSize)
    {
        ...
        if ((Config != null) && (Config.UseFullURL || (CMSContext.CurrentSiteID != LibraryInfo.LibrarySiteID) || (CMSContext.CurrentSiteID != GetCurrentSiteId())))
        {
