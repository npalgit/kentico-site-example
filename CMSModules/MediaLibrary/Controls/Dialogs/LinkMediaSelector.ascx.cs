    ...
    private void LoadDataSource(string searchText)
    {
        ...
        if ((FolderPath != null) && (LibraryID > 0))
        {
            ...
            if (err == "")
            {
                ...
                string where = String.Format("FilePath LIKE '{0}%' {{escape '{1}'}} AND FilePath NOT LIKE '{2}_%/%' {{escape '{1}'}} AND FileLibraryID = {3}", (String.IsNullOrEmpty(filePath) ? "" : filePath + "/"), PERCENT_ESC_CHAR, filePath, LibraryID);
