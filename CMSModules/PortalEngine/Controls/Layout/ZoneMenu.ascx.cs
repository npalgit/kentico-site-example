...
public partial class CMSModules_PortalEngine_Controls_Layout_ZoneMenu : CMSAbstractPortalUserControl
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!CMSContext.CurrentUser.IsAuthorizedPerResource("cms.contentpersonalization", "Manage"))
        {
            plcAddCPVariant.Visible = false;            
        }

        if (!CMSContext.CurrentUser.IsAuthorizedPerResource("cms.mvtest", "Manage"))
        {
        ...
        if (PortalContext.MVTVariantsEnabled || PortalContext.ContentPersonalizationEnabled)
        {
            ...
            if (PortalContext.MVTVariantsEnabled && (CMSContext.CurrentPageInfo != null) && (CMSContext.CurrentPageInfo.DocumentId > 0) && cui.IsAuthorizedPerResource("cms.mvtest","read"))
            {
            ...
            if ((PortalContext.ContentPersonalizationEnabled) && (CMSContext.CurrentPageInfo != null) && (CMSContext.CurrentPageInfo.DocumentId > 0) && cui.IsAuthorizedPerResource("cms.contentpersonalization", "read"))
            {
                // Set Display='none' for the MVT panel. Show dynamically only if required.
    ...
    protected void repZoneVariants_ItemDataBound(object sender, RepeaterItemEventArgs e)
    {
        ...
        if (pnlZoneVariantItem != null)
        {
            ...
            {
                lblZoneVariantItem.Text = HTMLHelper.HTMLEncode((((DataRowView)e.Item.DataItem)[columnVariantDisplayName]).ToString());
            }
    ...
    protected void menuZoneMVTVariants_OnReloadData(object sender, EventArgs e)
    {
        ...
        if ((CMSContext.CurrentUser == null)
            || (!CMSContext.CurrentUser.IsAuthorizedPerResource("CMS.MVTest", "Read")))
        {
            return;
        }
