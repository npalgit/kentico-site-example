...
public partial class CMSModules_PortalEngine_Controls_Layout_EditLayout : EditLayout
{
    ...
    protected override void OnPreRender(EventArgs e)
    {
        ...
        if (tmpCheckUser > 0)
        {
            ...
            if ((HttpContext.Current != null) && (tmpMachine.ToLower() == HTTPHelper.MachineName.ToLower()))
            {
                ...
                this.lblCheckOutInfo.Text = String.Format(ResHelper.GetString("PageLayout.CheckedOut", HTMLHelper.HTMLEncode(user.PreferredUICultureCode)), HttpContext.Current.Server.MapPath(tmpFilename));
            }
            else
            {
                this.lblCheckOutInfo.Text = String.Format(ResHelper.GetString("PageLayout.CheckedOutOnAnotherMachine", HTMLHelper.HTMLEncode(user.PreferredUICultureCode)), HTMLHelper.HTMLEncode(tmpMachine), HTMLHelper.HTMLEncode(username));
            }
        ...
        else
        {
            ...
            {
                this.lblCheckOutInfo.Text = String.Format(ResHelper.GetString("PageLayout.CheckOutInfo", HTMLHelper.HTMLEncode(user.PreferredUICultureCode)), HttpContext.Current.Server.MapPath(tmpLayoutUrl));
            }
    ...
    private bool SaveLayout()
    {
        ...
        if ((li != null) || (pti != null))
        {
            ...
            this.lblLayoutInfo.Text = ResHelper.GetString("General.ChangesSaved", HTMLHelper.HTMLEncode(culture));
