...
public partial class CMSModules_Membership_Pages_Users_User_Edit_General : CMSUsersPage
{
    ...
    protected void Page_PreRender(object sender, EventArgs e)
    {
        if (ui != null)
        {
            ...
            if (!ui.IsGlobalAdministrator || (CMSContext.CurrentUser.UserID == ui.UserID) || CMSContext.CurrentUser.UserSiteManagerDisabled)
            {
                // Only admin who have access to site manager can create another global admin
                if (CMSContext.CurrentUser.UserSiteManagerDisabled)
                {
    ...
    protected void btnOk_Click(object sender, EventArgs e)
    {
        ...
        if ((result == String.Empty) && (ui != null))
        {
            ...
            if ((uiTest == null) || (uiTest.UserID == userId))
            {
                ...
                if (!CMSContext.CurrentUser.UserSiteManagerDisabled)
                {
                    ...
                    // Only admin who have access to site manager can create another global admin
                    ui.IsGlobalAdministrator = CMSContext.CurrentUser.IsGlobalAdministrator && CheckBoxLabelIsGlobalAdministrator.Checked;
                }
        ...
        {
            if (CMSContext.CurrentUser.IsGlobalAdministrator && RequestHelper.IsFormsAuthentication() && (ui != null) && (ui.UserID != CMSContext.CurrentUser.UserID))
            {
                if (!CMSContext.CurrentUser.UserSiteManagerDisabled || !ui.UserIsGlobalAdministrator || ui.UserSiteManagerDisabled)
                {
                    string message = GetImpersonalMessage(ui);
