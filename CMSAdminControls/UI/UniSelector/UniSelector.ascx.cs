    ...
    private void LoadObjects(bool forceReload)
    {
        if (Object != null)
        {
            ...
            if (!String.IsNullOrEmpty(EditItemPageUrl))
            {
                ...
                javaScript.Append("function US_EditItem_", ClientID, "(selectedItem) {{ if (selectedItem == '') {{ alert('", GetString(ResourcePrefix + ".pleaseselectitem|general.pleaseselectitem"), "'); return false; }} var url = '", ResolveUrl(newUrl), "'; modalDialog(url.replace(/##ITEMID##/i, selectedItem),'", EditWindowName, "', ", EditDialogWindowWidth, ", ", EditDialogWindowHeight, "); }} \n");
            }
