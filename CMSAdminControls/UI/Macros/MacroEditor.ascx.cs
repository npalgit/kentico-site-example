...
public partial class CMSAdminControls_UI_Macros_MacroEditor : CMSUserControl, ICallbackEventHandler
{
    ...
    private static List<string> CommandsList
    {
        get
        {
            if (mMethodList == null)
            {
                ...
                mMethodList.Add("for@for (i = 0; i < 10; i++) { | }");
                mMethodList.Add("foreach@foreach (x in collection) { | }");
                mMethodList.Add("if@if (|) {  }");
                mMethodList.Add("while@while( | ) {  }");
                mMethodList.Add("break@break;");
                mMethodList.Add("continue@continue;");
                mMethodList.Add("return@return;");
                mMethodList.Add("print@print(|);");
                mMethodList.Add("println@println(|);");
                mMethodList.Add("null@null");
            }
            return mMethodList;
        }
    }
            ...
            if (mUsingList == null)
            {
                ...
                if (this.NamespaceUsings != null)
                {
                    foreach (string name in this.NamespaceUsings)
                    {
                        ...
                        {
                            List<MacroMethod> methods = MacroMethods.GetMethodsForObject(namespaceInstance, false);
            ...
            if (brackets == 0)
            {
                ...
                if (type == ElementType.LeftBracket)
                {
                    if ((i > 0) && (tokens[i - 1].Type == ElementType.Identifier))
                    {
                        ...
                        if (i > 1)
                        {
                            withoutFirstParam = (tokens[i - 2].Type == ElementType.Dot);
                        }
                    }
                    break;
                }
        ...
        foreach (MacroMethod method in methodList)
        {
            ...
            if (!MacroElement.IsValidOperator(m) && !MacroElement.IsWordOperator(m))
            {
                if ((m != "logtodebug") || CMSContext.CurrentUser.IsGlobalAdministrator)
                {
                    ...
                    {
                        autoCompletionList.Add(method.Name + "@" + method.Snippet);
                    }
                }
            }
        }
    ...
    private string GetJSMethodsHashTable()
    {
        ...
        foreach (MacroMethod m in MacroMethods.Methods.Values)
        {
            ...
            sb.AppendLine("methodComments[" + ScriptHelper.GetString(name + "@") + "] = " + ScriptHelper.GetString(comment) + ";");
        }
        return sb.ToString();
    }
