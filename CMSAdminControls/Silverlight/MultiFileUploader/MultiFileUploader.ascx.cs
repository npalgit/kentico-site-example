...
public partial class CMSAdminControls_Silverlight_MultiFileUploader_MultiFileUploader : CMSUserControl
{
    ...
    public int ResizeToHeight
    {
        get
        {
            if (mResizeToWidth == -1)
            {
                mResizeToWidth = SettingsKeyProvider.GetIntValue(CMSContext.CurrentSiteName + ".CMSAutoResizeImageHeight");
            }
    ...
    public int ResizeToMaxSideSize
    {
        get
        {
            if (mResizeToWidth == -1)
            {
                mResizeToWidth = SettingsKeyProvider.GetIntValue(CMSContext.CurrentSiteName + ".CMSAutoResizeImageMaxSideSize");
            }
    ...
    private string GetAdditionalParameters()
    {
        ...
        return "AdditionalParameters=" + string.Join(PARAMETER_SEPARATOR, args);
    }
    ...
    private string GetModeParameters()
    {
        ...
        if (MediaLibraryID > 0)
        {
            ...
            return "MediaLibraryArgs=" + string.Join(PARAMETER_SEPARATOR, args);
        }
        else
        {
            if ((NodeID > 0) && (SourceType == MediaSourceEnum.Content))
            {
                ...
                return "FileArgs=" + string.Join(PARAMETER_SEPARATOR, args);
            }
            else
            {
                if (ObjectID > 0)
                {
                    ...
                    "Category", Category };
                    return "MetaFileArgs=" + string.Join(PARAMETER_SEPARATOR, args);
                }
                else
                {
                    if (PostForumID > 0)
                    {
                        ...
                        return "ForumArgs=" + string.Join(PARAMETER_SEPARATOR, args);
                    }
                    else
                    {
                        if ((DocumentID > 0) || (FormGUID != Guid.Empty))
                        {
                            ...
                            return "AttachmentArgs=" + string.Join(PARAMETER_SEPARATOR, args);
                        }
                    }
                }
            }
    ...
    private string GetRestrictionParameters()
    {
        ...
        Multiselect, MaxNumberToUpload, MaximumTotalUpload, MaximumUpload, MaxConcurrentUploads, UploadChunkSize);
    }
    ...
    private string GetFilterParameters()
    {
        ...
        if (!string.IsNullOrEmpty(filter))
        {
            ...
            filter = "Filter=Allowed files|*." + filter;
        }
