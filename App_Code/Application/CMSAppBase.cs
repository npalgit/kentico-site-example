...
public class CMSAppBase
{
        ...
        if (ConnectionAvailable)
        {
            ...
            if (h.Continue)
            {
                ...
                if (siteName != "")
                {
                    ...
                    if (AnalyticsHelper.IsLoggingEnabled(siteName, String.Empty))
                    {
    ...
    public static void CMSAcquireRequestState(object sender, EventArgs e)
    {
        ...
        if (h.Continue)
        {
            ...
            // Log page view
            if (ConnectionAvailable && !RequestHelper.IsPostBack() && (CMSContext.ViewMode == ViewModeEnum.LiveSite) && (status != URLRewritingResultEnum.RESTService))
            {
                string siteName = CMSContext.CurrentSiteName;
                PageInfo currentPage = CMSContext.CurrentPageInfo;

                if (!String.IsNullOrEmpty(siteName) && (currentPage != null))
                {
                    if (!QueryHelper.GetBoolean(URLHelper.SYSTEM_QUERY_PARAMETER, false))
                    {
                        // Log landing page, and referring sites
        ...
        if (h.Continue)
        {
            ...
            if (ConnectionAvailable)
            {
                ...
                if (RequestHelper.LogPageView)
                {
                    ...
                    if (ActivitySettingsHelper.ActivitiesEnabledAndModuleLoaded(siteName) && ActivitySettingsHelper.ActivitiesEnabledForThisUser(CMSContext.CurrentUser) &&
                        ActivitySettingsHelper.PageVisitsEnabled(siteName))
                    {
        ...
        if (ConnectionAvailable)
        {
            if (HttpContext.Current != null)
            {
                ...
                if (ex != null)
                {
                    ...
                    // In case of 404 exception check if it can be logged
                    if ((ex is HttpException) && (URLRewriter.CurrentStatus == URLRewritingResultEnum.PageNotFound))
                    {
                        eventCode = "PAGENOTFOUND";
                        eventType = EventLogProvider.EVENT_TYPE_WARNING;

                        if (!SettingsKeyProvider.GetBoolValue(CMSContext.CurrentSiteName + ".CMSLogPageNotFoundException"))
                        {
                            log = false;
                        }
                    }
                        ...
                        try
                        {
                            ...
                            if (logException)
                            {
                                ...
                                try
                                {
                                    ...
                                    eProvider.LogEvent(eventType, DateTime.Now, "Application_Error", eventCode, CMSContext.CurrentUser.UserID, CMSContext.CurrentUser.UserName, (CMSContext.CurrentDocument != null) ? CMSContext.CurrentDocument.NodeID : 0, (CMSContext.CurrentDocument != null) ? CMSContext.CurrentDocument.DocumentName : null, HTTPHelper.GetUserHostAddress(), EventLogProvider.GetExceptionLogMessage(ex), CMSContext.CurrentSiteID, HTTPHelper.GetAbsoluteUri());
                                }
                                catch
                                {
                                    // can't write to log, do not process any code
