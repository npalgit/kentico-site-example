...
public class ResourceHandler : IHttpHandler
{
    ...
    /// <summary>
    /// Supported querystring argument used to identify css files.
    /// </summary>
    private const string CSS_FILE_ARGUMENT = "stylesheetfile";
    ...
    /// <summary>
    /// Supported querystring argument used to identify stylesheets stored in a database.
    /// </summary>
    private const string CSS_DATABASE_ARGUMENT = "stylesheetname";


    /// <summary>
    /// Supported querystring argument used to identify web part container stylesheets stored in a database.
    /// </summary>
    private const string CONTAINERS_DATABASE_ARGUMENT = "containers";
    ...
    private static CMSOutputResource CombineResources(List<CMSOutputResource> resources)
    {
        ...
        foreach (CMSOutputResource resource in resources)
        {
            if (resource != null)
            {
                ...
                if ((data.Length > 0) || !String.IsNullOrEmpty(resource.Data))
                {
                    data.AppendLine();
                    data.AppendLine();
                }
    ...
    /// <summary>
    /// Processes a request for CSS file identified by its URL.
    /// </summary>
    /// <param name="context">An HTTPContext object that provides references to the intrinsic server objects used to service HTTP requests</param>
    private static void ProcessCSSFileRequest(HttpContext context)
    {
        ProcessFileRequest(context, CSS_FILE_ARGUMENT, CSS_FILE_EXTENSION, mCssMinifier, MimeTypeHelper.GetMimetype(CSS_FILE_EXTENSION), CSSHelper.StylesheetMinificationEnabled);
    }
    ...
    private static void ProcessFileRequest(HttpContext context, string queryArgument, string fileExtension, IResourceMinifier minifier, string contentType, bool minificationEnabled)
    {
        ...
        // Try to get data from cache (or store them if not found)
        using (CachedSection<CMSOutputResource> cachedSection = new CachedSection<CMSOutputResource>(ref resource, PhysicalFilesCacheMinutes, true, null, "getresource", path))
        {
            // Not found in cache; load the data
            if (cachedSection.LoadData)
            {
