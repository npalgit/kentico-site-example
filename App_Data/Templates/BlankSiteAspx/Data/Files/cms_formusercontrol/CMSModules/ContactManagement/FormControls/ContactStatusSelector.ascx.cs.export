...
public partial class CMSModules_ContactManagement_FormControls_ContactStatusSelector : FormEngineUserControl
{
    ...
    /// <summary>
    /// Indicates if current control is being used in site manager by global admin.
    /// </summary>
    ...
    public void ReloadData()
    {
        ...
        if (IsSiteManager != null)
        {
            allowGlobal = ConfigurationHelper.AuthorizedReadConfiguration(UniSelector.US_GLOBAL_RECORD, false, (bool)IsSiteManager);
        }
        else
        {
            allowGlobal = ConfigurationHelper.AuthorizedReadConfiguration(UniSelector.US_GLOBAL_RECORD, false);
        }
        if (this.SiteID > 0)
        {
            allowSite = ConfigurationHelper.AuthorizedReadConfiguration(this.SiteID, false);
        }
        else
        {
        ...
        uniselector.AllowAll = this.AllowAllItem;
