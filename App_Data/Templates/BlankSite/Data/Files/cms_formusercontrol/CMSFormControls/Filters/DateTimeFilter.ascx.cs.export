...
public partial class CMSFormControls_Filters_DateTimeFilter : FormEngineUserControl, IFilterFormControl
{
    ...
    public override bool IsValid()
    {
        ...
        // If filed doesn't allow null
        if ((((this.FieldInfo != null) && !this.FieldInfo.AllowEmpty)
            || !String.IsNullOrEmpty(strValueFrom) || !String.IsNullOrEmpty(strValueTo))
            // And value 'from' is not valid time
            && (((ValidationHelper.GetDateTime(strValueFrom, DateTimeHelper.ZERO_TIME) == DateTimeHelper.ZERO_TIME) && String.IsNullOrEmpty(strValueTo))
            // Or value 'to' is not valid time
            || ((ValidationHelper.GetDateTime(strValueTo, DateTimeHelper.ZERO_TIME) == DateTimeHelper.ZERO_TIME) && String.IsNullOrEmpty(strValueFrom))))
        {
