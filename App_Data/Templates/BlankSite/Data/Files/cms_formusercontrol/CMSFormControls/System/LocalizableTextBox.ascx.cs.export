...
public partial class CMSFormControls_System_LocalizableTextBox : FormEngineUserControl, ICallbackEventHandler
{
    ...
    public override object Value
    {
        ...
        set
        {
            hdnValue.Value = ValidationHelper.GetString(value, null);

            // Check if value is localization macro
            if (!String.IsNullOrEmpty(hdnValue.Value) && hdnValue.Value.StartsWith(MACRO_START) && hdnValue.Value.EndsWith(MACRO_END))
            {
                this.IsMacro = true;
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        bool enableUI = 
            ...
            !this.IsLiveSite && 
            (SettingsKeyProvider.DevelopmentMode || CultureInfoProvider.IsSiteMultilignual(CMSContext.CurrentSiteName));

        if (enableUI)
        {
            // Set controls
            btnOtherLanguages.ImageUrl = GetImageUrl("Objects/CMS_UICulture/list.png");
            btnOtherLanguages.ToolTip = GetString("localizable.otherlanguages");
            btnOtherLanguages.Attributes.Add("onClick", "LocalizeString('" + hdnValue.ClientID + "', '" + textbox.ClientID + "'); return false;");

            btnRemoveLocalization.Attributes.Add("onClick", "return confirm('" + GetString("localizable.removelocalization") + "');");
            btnRemoveLocalization.ImageUrl = GetImageUrl("Design/Controls/UniGrid/Actions/Delete.png");
            btnRemoveLocalization.ToolTip = GetString("localizable.remove");
            btnRemoveLocalization.Click += new ImageClickEventHandler(btnRemoveLocalization_Click);

            btnLocalize.ImageUrl = GetImageUrl("Objects/CMS_UICulture/add.png");
            btnLocalize.ToolTip = GetString("localizable.localize");

            // In automatic mode resource string key is generated from plain text
            if (this.AutomaticMode)
            {
                btnLocalize.Click += new ImageClickEventHandler(btnLocalize_Click);
            }
            // Otherwise user has option to select resource string key
            else
            {
                btnLocalize.Attributes.Add("onClick", "LocalizationDialog" + ClientID + "('$|' + document.getElementById('" + textbox.ClientID + "').value); return false;");
            }
        ...
        if (RequestHelper.IsPostBack() && this.AutoSave)
        {
            Save();
        }
    }
    ...
    public void Reload()
    {
        if (this.pnlButtons.Visible)
        {
            ...
            if (this.IsMacro && this.LocalizationExists)
            {
                btnLocalize.Attributes.Add("style", "display: none;");
                btnOtherLanguages.Attributes.Add("style", "display: inline;");
                btnRemoveLocalization.Attributes.Add("style", "display: inline;");
            }
            // Textbox contains only plain text
            else
            {
                btnOtherLanguages.Attributes.Add("style", "display: none;");
                btnRemoveLocalization.Attributes.Add("style", "display: none;");
                btnLocalize.Attributes.Add("style", "display:inline;");
            }
        }
    }
...
"
    ...
    /// </summary>
    void btnLocalize_Click(object sender, ImageClickEventArgs e)
    {
    ...
    /// </summary>
    void btnRemoveLocalization_Click(object sender, ImageClickEventArgs e)
    {
