...
function AutoCompleteExtender(codeElem, hintElem, contextElem, quickContextElem, mixedMode, editorTopOffset, editorLeftOffset) {
    ...
    this.showContext = function(charCode) {
        if (!this.isInsideComment()) {
            ...
            if (this.isMixedMode) {
                ...
                currentLineMacro = this.currentLineText() + '@@' + this.currentLinePos();
            }
            if (charCode == null) {
                this.callbackArgument = currentLineMacro + '@context@';
            } else {
                this.callbackArgument = currentLineMacro.replace('@@', '@' + charCode + '@') + '@context@';
            }
    ...
    */
    ...
    this.showHints = function(charCode) {
        if (!this.isInsideComment()) {
            if (!this.isMixedMode) {
                ...
                if (charCode == null) {
                    this.callbackArgument = this.currentLineText() + '@@' + this.currentLinePos() + '@hint@' + prevText;
                } else {
                    this.callbackArgument = this.currentLineText() + '@' + charCode + '@' + this.currentLinePos() + '@hint@' + prevText;
                }
            } else {
                // If the mode is mixed, parse only macro text and return only relevant parts of the code
                ...
                if (charCode == null) {
                    this.callbackArgument = currentLineMacro + '@hint@' + prevMacros;
                } else {
                    this.callbackArgument = currentLineMacro.replace('@@', '@' + charCode + '@') + '@hint@' + prevMacros;
                }
            }
    ...
    this.useHint = function(isTabUsage) {
        if ((this.hints.length > 0) && (this.highlighted > -1)) {
            ...
            var pos = { line: currCursorLine, ch: pos1 + hintToUse.length - offset };
    ...
    */
    ...
    this.getCurrentLineMacro = function() {
        ...
        return currentLineMacro + '@@' + caretPos;
    }
